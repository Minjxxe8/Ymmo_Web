...
const STATUS_LABELS = { completed: 'Validé', pending: 'En cours', failed: 'Échoué' }
            ...
            >
                <div className="flex items-center gap-4">
                    <div className="w-10 h-10 flex items-center justify-center shrink-0"
                         style={{ backgroundColor: `${TYPE_COLORS[tx.type]}18` }}>
              ...
              {tx.type === 'deposit' ? '↓' : tx.type === 'withdrawal' ? '↑' : '🏠'}
                    ...
                    <div>
                        <p className="text-sm font-medium" style={{ color: '#0D1F3C' }}>
                            {tx.propertyTitle ?? TYPE_LABELS[tx.type]}
                        </p>
                ...
                <div className="flex items-center gap-4">
          <span className="text-xs px-2 py-1" style={{ backgroundColor: STATUS_BG[tx.status], color: STATUS_COLOR[tx.status] }}>
            {STATUS_LABELS[tx.status]}
          </span>
                    <span className="text-base font-semibold w-28 text-right"
                          style={{ color: tx.amount > 0 ? '#009886' : '#0D1F3C' }}>
            {tx.amount > 0 ? '+' : ''}{formatPrice(tx.amount)}
          </span>
                    <span className="text-xs ml-2 transition-transform duration-200"
                          style={{ color: '#C5C6CE', transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)', display: 'inline-block' }}>
            ▾
          </span>
                </div>
            </button>
                ...
                <div className="px-4 pb-5 pt-1">
                    <div className="border border-gray-100 p-5" style={{ backgroundColor: '#F9F7F3' }}>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            ...
                            <div>
                                <p className="text-xs tracking-widest uppercase mb-1" style={{ color: '#C5C6CE' }}>Type</p>
                                <p className="text-sm font-medium">{TYPE_LABELS[tx.type]}</p>
                            </div>
                            <div>
                                <p className="text-xs tracking-widest uppercase mb-1" style={{ color: '#C5C6CE' }}>Montant</p>
                                <p className="text-sm font-semibold" style={{ color: tx.amount > 0 ? '#009886' : '#0D1F3C' }}>
                                    {tx.amount > 0 ? '+' : ''}{formatPrice(tx.amount)}
                                </p>
                            </div>
                            <div>
                                <p className="text-xs tracking-widest uppercase mb-1" style={{ color: '#C5C6CE' }}>Statut</p>
                                <span className="text-xs px-2 py-1" style={{ backgroundColor: STATUS_BG[tx.status], color: STATUS_COLOR[tx.status] }}>
                  {STATUS_LABELS[tx.status]}
                </span>
                            </div>
                            ...
                            {tx.propertyTitle && (
                                <>
                                    <div>
                                        <p className="text-xs tracking-widest uppercase mb-1" style={{ color: '#C5C6CE' }}>Bien</p>
                                        <p className="text-sm font-medium">{tx.propertyTitle}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs tracking-widest uppercase mb-1" style={{ color: '#C5C6CE' }}>Ville</p>
                                        <p className="text-sm">{tx.propertyCity}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs tracking-widets uppercase mb-1" style={{ color: '#C5C6CE' }}>Agent</p>
                                        <p className="text-sm">{tx.agentName}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs tracking-widest uppercase mb-1" style={{ color: '#C5C6CE' }}>Contact</p>
                                        <a href={`mailto:${tx.agentEmail}`} className="text-xs hover:underline" style={{ color: '#009886' }}>
                                            {tx.agentEmail}
                                        </a>
                                    </div>
                                </>
                            ...
                            {tx.note && (
                                <div className="col-span-2 md:col-span-4">
                                    <p className="text-xs tracking-widest uppercase mb-1" style={{ color: '#C5C6CE' }}>Note</p>
            ...
            <div className="bg-white p-8 w-full max-w-sm shadow-xl">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1">
                        <label className="text-xs tracking-widest uppercase" style={{ color: '#44474D' }}>Montant (€)</label>
                        <input
                            type="number"
                            min="1"
                            step="100"
...
export default function WalletPage() {
    ...
    const handleDeposit = async (amount) => {
        const updated = await deposit({ amount })
        setWallet(updated)
    }

    const handleWithdraw = async (amount) => {
        const updated = await withdraw({ amount })
        setWallet(updated)
    }

    const totalDeposits = transactions.filter((t) => t.type === 'deposit').reduce((acc, t) => acc + t.amount, 0)
    const totalPurchases = transactions.filter((t) => t.type === 'purchase').reduce((acc, t) => acc + Math.abs(t.amount), 0)
        ...
        <div className="min-h-screen" style={{ backgroundColor: '#FCF9F4', fontFamily: "'Space Grotesk', sans-serif", color: '#0D1F3C' }}>
            <div className="max-w-5xl mx-auto px-8 lg:px-16 py-16">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-12">
                    <div className="lg:col-span-1 p-8 flex flex-col justify-between" style={{ backgroundColor: '#0D1F3C', minHeight: '200px' }}>
                        <div>
                            <p className="text-xs tracking-widest uppercase mb-6" style={{ color: '#C5C6CE' }}>Solde disponible</p>
                    ...
                    <div className="flex flex-col gap-4">
                        <button onClick={() => setModal('deposit')}
                            ...
                            <span className="text-xs tracking-widest uppercase" style={{ color: '#009886' }}>Déposer des fonds</span>
                        </button>
                        <button onClick={() => setModal('withdrawal')}
                                className="flex-1 flex flex-col items-center justify-center gap-2 border-2 border-dashed hover:bg-gray-50 transition-colors py-6"
                                style={{ borderColor: '#C9A84C' }}>
                            <span className="text-2xl" style={{ color: '#C9A84C' }}>↑</span>
                            <span className="text-xs tracking-widest uppercase" style={{ color: '#C9A84C' }}>Retirer des fonds</span>
                        </button>
                    </div>

                    <div className="flex flex-col gap-4">
                        {[
                            ['Total déposé', totalDeposits, '#009886'],
                            ['Total dépensé', totalPurchases, '#C9A84C'],
                            ['Transactions', transactions.length, '#0D1F3C'],
                        ].map(([label, value, color]) => (
                            <div key={label} className="flex-1 border border-gray-200 p-4 flex flex-col justify-between" style={{ backgroundColor: 'white' }}>
                                ...
                                <p className="text-xl font-semibold mt-2" style={{ color }}>
                                    {typeof value === 'number' && label !== 'Transactions'
                ...
                <div>
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <span className="text-xs tracking-widets uppercase px-3 py-1 border" style={{ borderColor: '#C5C6CE', color: '#44474D' }}>
                    ...
                    <div className="bg-white border border-gray-100">
                        {transactions.length === 0 ? (
                            <div className="py-16 text-center">
                                <p className="text-xl italic mb-2" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Aucune transaction</p>
