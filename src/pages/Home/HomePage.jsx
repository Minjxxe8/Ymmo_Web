...
import {getProperties, getPropertyTypes, searchProperties} from '../../api/properties.api'
...
function PropertyCard({property, isFav, onToggleFav, user}) {
    return (<div className="bg-white group h-full">
        <div className="relative overflow-hidden" style={{aspectRatio: '16/10', backgroundColor: '#E8E4DC'}}>
            {property.picturePath ? (<img src={property.picturePath} alt={property.name}
                                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"/>) : (
                <div className="w-full h-full group-hover:scale-105 transition-transform duration-500"
                     style={{backgroundColor: '#D4CFC7'}}/>)}
            {!property.onSale && (
                <span className="absolute top-3 left-3 text-xs tracking-widest uppercase px-3 py-1 text-white"
                      style={{backgroundColor: '#44474D'}}>Vendu</span>)}
            {user && (<button
                onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
        ...
        <div className="p-5">
            <div className="flex justify-between items-start mb-2">
                <div>
                    ...
                    <h3 className="text-lg italic group-hover:text-yellow-600 transition-colors"
                        style={{fontFamily: "'Cormorant Garamond', serif", color: '#0D1F3C'}}>{property.name}</h3>
                </div>
                <span className="text-base font-semibold whitespace-nowrap ml-4"
                      style={{color: '#C9A84C'}}>{formatPrice(property.price)}</span>
            </div>
            <div className="flex gap-6 pt-4 border-t border-gray-100">
                ...
                <div>
                    ...
                    <span className="inline-block text-xs font-bold text-white px-2 py-0.5"
...
export default function HomePage() {
    ...
    useEffect(() => {
        const params = {}
        params.query = filters.city ?? ''

        if (filters.type && filters.type !== 'Tous') {
            const maybeId = Number(filters.type)
            if (!isNaN(maybeId)) params.typeId = maybeId
            else {
                const found = types.find((t) => t.name === filters.type)
                if (found) params.typeId = found.id
            }
        }
        ...
        if (params.query !== undefined && !params.q) params.q = params.query
        ...
        Promise.all([
            ...
            setProperties(props)
            setFavorites(favs.map((f) => (typeof f === 'string' ? f : (f.property?.id ?? f.id ?? f))))
    ...
    return (<div style={{backgroundColor: '#FCF9F4', fontFamily: "'Space Grotesk', sans-serif", color: '#0D1F3C'}}>

        <section className="py-12 border-b border-gray-200">
            <div className="max-w-7xl mx-auto px-8 lg:px-16">
                <h1 className="text-4xl lg:text-5xl italic mb-8"
                ...
                <div className="flex flex-wrap gap-6 items-end">
                    <div className="flex flex-col gap-1">
                        <label className="text-xs tracking-widest uppercase"
                               style={{color: '#44474D'}}>Ville, Quartier, Nom...</label>
                        <input
                            value={cityInput}
                            onChange={(e) => setCityInput(e.target.value)}
                            placeholder="Toutes les villes, quarties, noms..."
        ...
        <section className="py-12">
            <div className="max-w-7xl mx-auto px-8 lg:px-16">
                {loading ? (<div className="flex justify-center py-24">
                    <div
                        className="w-8 h-8 border-2 border-gray-200 border-t-[#C9A84C] rounded-full animate-spin"/>
                </div>) : properties.length === 0 ? (<div className="text-center py-24">
                    <p className="text-xl italic mb-3" style={{fontFamily: "'Cormorant Garamond', serif"}}>Aucun
                        bien trouvé</p>
                    <p className="text-sm" style={{color: '#44474D'}}>Essayez d'élargir vos critères de
                        recherche.</p>
                </div>) : (<>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {properties.map((p) => (/* ---> MODIFICATION ICI : On wrap avec un composant Link <--- */
                            <Link
                                to={`/property/${p.id}`}
                                key={p.id}
        ...
        {!user && (<section className="py-20" style={{backgroundColor: '#0D1F3C'}}>
            <div className="max-w-2xl mx-auto px-8 text-center">
                <h2 className="text-4xl italic mb-4 text-white"
