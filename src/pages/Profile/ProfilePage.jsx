...
function FavoriteCard({ property }) {
    ...
    return (
        <div onClick={() => navigate(`/property/${property.id}`)}
            ...
            <div className="shrink-0 overflow-hidden" style={{ width: '80px', height: '60px', backgroundColor: '#E8E4DC' }}>
                <div className="w-full h-full group-hover:scale-105 transition-transform duration-500" style={{ backgroundColor: '#D4CFC7' }} />
            </div>
            <div className="flex-1 min-w-0">
                <p className="text-xs tracking-widest uppercase mb-0.5" style={{ color: '#44474D' }}>{property.type} · {property.city}</p>
                <h4 className="text-sm italic truncate" style={{ fontFamily: "'Cormorant Garamond', serif", color: '#0D1F3C' }}>{property.title}</h4>
                <p className="text-sm font-semibold mt-1" style={{ color: '#C9A84C' }}>{formatPrice(property.price)}</p>
            </div>
            {property.dpe && (
                <div className="shrink-0 flex items-start pt-1">
          <span className="text-xs font-bold text-white px-1.5 py-0.5" style={{ backgroundColor: DPE_COLORS[property.dpe] }}>
            {property.dpe}
          </span>
                </div>
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(13,31,60,0.7)' }}>
            <div className="bg-white p-8 w-full max-w-md shadow-xl">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl italic" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Modifier mes informations</h3>
        ...
        <div className="min-h-screen" style={{ backgroundColor: '#FCF9F4', fontFamily: "'Space Grotesk', sans-serif", color: '#0D1F3C' }}>
            <div className="max-w-5xl mx-auto px-8 lg:px-16 py-16">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    ...
                    <div className="lg:col-span-1 flex flex-col gap-6">
                        ...
                        <div className="bg-white border border-gray-100 p-8 flex flex-col items-center text-center">
                            <div className="w-24 h-24 flex items-center justify-center text-white text-3xl font-semibold mb-4"
                ...
                {ROLE_LABELS[user?.role] ?? user?.role}
                            ...
                            {user?.phone && (
                                <p className="text-xs mb-4" style={{ color: '#C5C6CE' }}>{user.phone}</p>
                            )}
                        ...
                        <div className="bg-white border border-gray-100 p-6">
                            <p className="text-xs tracking-widest uppercase mb-4" style={{ color: '#C5C6CE' }}>Portefeuille</p>
                  ...
                  {favorites.length} bien{favorites.length > 1 ? 's' : ''}
                            ...
                            {favorites.length === 0 ? (
                                <div className="bg-white border border-gray-100 py-12 text-center">
                                    <p className="text-xl italic mb-2" style={{ fontFamily: "'Cormorant Garamond', serif" }}>Aucun favori</p>
                                    <p className="text-xs mb-4" style={{ color: '#C5C6CE' }}>Ajoutez des biens à vos favoris depuis les annonces</p>
                                    <Link to="/home" className="text-xs tracking-widest uppercase hover:underline" style={{ color: '#C9A84C' }}>
                                        Voir les annonces →
                                    </Link>
                                </div>
                            ) : (
                                <div className="flex flex-col gap-3">
                                    {favorites.map((p) => <FavoriteCard key={p.id} property={p} />)}
                                </div>
                            )}
                        </div>
