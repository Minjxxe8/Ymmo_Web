...
export default function LandingPage() {
    ...
    return (<div style={{backgroundColor: '#FCF9F4', fontFamily: "'Space Grotesk', sans-serif", color: '#0D1F3C'}}>
        ...
        <section className="relative min-h-screen flex flex-col">
            ...
            <div
                className="relative z-20 flex-1 flex flex-col justify-center items-start max-w-7xl mx-auto w-full px-8 lg:px-16 pt-32 pb-24">
                    ...
                    <div className="mb-4">
                        <div className="flex justify-between mb-1">
                            ...
                            <span className="text-xs font-medium"
                                  style={{color: '#C9A84C'}}>{formatPriceShort(search.maxPrice)}</span>
                        </div>
        ...
        <section className="py-24 max-w-7xl mx-auto px-8 lg:px-16">
            ...
            <h2 className="text-4xl lg:text-5xl italic mb-12"
                style={{fontFamily: "'Cormorant Garamond', 'Liberation Serif', serif"}}>
                Biens en vedette
            </h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {Array.isArray(featured) && featured.map((p) => (
                    <Link to={"/property/" + p.id} key={p.id} className="group cursor-pointer">
                        <div className="relative overflow-hidden mb-4"
                             style={{aspectRatio: '4/3', backgroundColor: '#E8E4DC'}}>
                            {p.picturePath ? (
                                <img src={p.picturePath} alt={p.name}
                        ...
                        <div className="flex justify-between items-start mb-1">
                            <div>
                                <p className="text-xs tracking-widest uppercase mb-1"
                                   style={{color: '#44474D'}}>{p.type.name} · {p.city}</p>
                                <h3 className="text-xl italic"
                                    style={{fontFamily: "'Cormorant Garamond', serif"}}>{p.name}</h3>
                            </div>
                        ...
                        <div className="flex gap-6 mt-3">
                            <div>
                                <p className="text-xs tracking-widest uppercase"
                                   style={{color: '#C5C6CE'}}>Surface</p>
                                <p className="text-sm font-medium">{p.surfaceArea} m²</p>
                            </div>
                            <div>
                                ...
                                <span className="inline-block text-xs font-bold text-white px-2 py-0.5"
                                      style={{backgroundColor: DPE_COLORS[p.diagnostic]}}>
                                {p.diagnostic}
                                </span>
                            </div>
                        </div>
        ...
        {!user && (<section className="py-24" style={{backgroundColor: '#0D1F3C'}}>
